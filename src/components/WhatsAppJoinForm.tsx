...
export default function WhatsAppJoinForm({ dict }: WhatsAppJoinFormProps) {
  ...
  return (
    <div className={`bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl shadow-lg border border-green-100 transition-all duration-300 flex flex-col h-full ${isExpanded ? 'p-8' : 'p-6'}`}>
      <div className={`flex items-center gap-3 mb-4 ${!isExpanded ? 'flex-1' : ''}`}>
      ...
      {!isExpanded ? (
        <button
          ...
          className="w-full bg-green-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-green-700 transition flex items-center justify-center gap-2 mt-auto"
        >
