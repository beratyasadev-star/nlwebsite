...
function serializeContent(content: any): string {
  ...
  return content.map((node: any) => {
    ...
    if (node.type === 'li') return `<li class="ml-4">${text}</li>`
    
    return `<p class="mb-2">${text}</p>`
  }).join('')
}

function SSSClient({ faqs }: SSSPageProps) {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 pl-12 rounded-lg border border-gray-300 focus:ring-2 focus:ring-sky-500 focus:border-transparent"
            />
        ...
        <div className="space-y-3">
          {filteredFAQs.length === 0 ? (
            <div className="text-center py-16 bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-sm border border-gray-100">
              ...
              <p className="text-gray-500 text-lg">Bu kategoride soru bulunmamaktadır.</p>
            </div>
          ) : (
            filteredFAQs.map((faq) => (
              <div key={faq.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100">
                <button
                  ...
                  className="w-full px-6 py-5 text-left flex justify-between items-start gap-4 hover:bg-gradient-to-r hover:from-sky-50 hover:to-transparent transition-all duration-200"
                >
        ...
        <div className="mt-12 bg-sky-50 rounded-lg p-8 text-center">
          ...
          <p className="text-gray-700 mb-6">
            Sorunuzu bize iletebilir veya WhatsApp grubumuzda diğer üyelerle paylaşabilirsiniz.
          </p>
